...
def authenticate_github(token: str) -> Github:
    """Authenticate with GitHub using a token or GITHUB_TOKEN.

    Args:
        token: GitHub personal access token, fine-grained token, or GITHUB_TOKEN.

    Returns:
        An authenticated PyGithub client instance.
    ...
    # Avoid calling `github.get_user().login` here, as GitHub Actions GITHUB_TOKEN
    # does not have permissions to access the GET /user endpoint and will raise HTTP 403.
    github = Github(auth=Auth.Token(token.strip()))
    LOGGER.info("Authenticated PyGithub client successfully initialized.")
    return github
